...
```{r}
 record_path <- function (chunk = NULL,
   ...
   LOGO$path <- rbind(LOGO$path, newrows)
   rownames(LOGO$path) <- NULL
   LOGO$pos <- nrow(LOGO$path)
}
```
...
plot_path <- function (stat) {
  if (stat > 1) {
     for (i in 2:stat) {
       segments(x0 = LOGO$path$x[i-1], y0 = LOGO$path$y[i-1],
                x1 = LOGO$path$x[i], y1 = LOGO$path$y[i],
                col = LOGO$path$path_color[i], lwd = 2)
     }
  }
}
```
...
plot_turtle <- function (stat) {
  ...
  if (show) {
    ...
    arrows(LOGO$path$x[stat],
           LOGO$path$y[stat],
           LOGO$path$x[stat] + cos(angle) * arrow_length,
           LOGO$path$y[stat] + sin(angle) * arrow_length,
